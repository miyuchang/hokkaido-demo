...
export const CostView: React.FC<CostViewProps> = ({ expenses, isLoading, onRefresh, onAddSuccess }) => {
  ...
  const handleOpenEdit = (e: React.MouseEvent, record: ExpenseRecord) => {
    ...
    const amt = isTWD ? record.amountTwd : record.amountJpy;
  ...
  return (
    <div className="pb-32 pt-1 animate-in fade-in duration-700">
      <div className="bg-white border border-gray-200 rounded-none overflow-hidden mb-8 shadow-sm">
        ...
        <button onClick={() => setShowSettlement(true)} className="w-full bg-mag-black text-white py-3.5 font-bold text-sm tracking-[0.2em] active:bg-mag-gold transition-colors">結算精算 SETTLE</button>
      </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-sm p-8 z-10 border-t-8 border-mag-black animate-in zoom-in-95">
             <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setShowSettlement(false)} className="text-gray-300 p-1"><XIcon className="w-6 h-6" /></button>
             </div>
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex border-[1.5px] border-mag-black h-[42px] overflow-hidden">
                ...
                <div>
                  ...
                  <div className="relative flex bg-[#F7F7F7] items-center px-3 h-[42px] border border-transparent focus-within:border-mag-gold overflow-hidden">
                      ...
                      <div className="flex bg-white shadow-sm border border-gray-200 h-[28px] shrink-0 ml-2 overflow-hidden">
                          <button type="button" onClick={() => setCurrency('JPY')} className={`px-2.5 font-bold text-[9px] transition-colors border-r border-gray-100 ${currency === 'JPY' ? 'bg-mag-black text-white' : 'text-gray-400 hover:bg-gray-50'}`}>JPY</button>
                ...
                <div className="bg-[#F8F8F8] p-3 space-y-3">
                    <div className="flex justify-between items-center mb-0.5">
                      <label className="text-[10px] font-black text-mag-black uppercase tracking-wider">分帳模式 SPLIT</label>
                      <select 
                        value={splitType} 
                      ...
                      >
                          <option value="equal">平均分攤 (1/2)</option>
                          <option value="manual">自定義金額</option>
                      </select>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className={`text-[8px] font-black mb-1 block uppercase tracking-tighter ${splitType === 'manual' ? 'text-[#E91E63]' : 'text-gray-400'}`}>小 A 應付</label>
                          <input 
                            type="number" 
                            disabled={splitType === 'equal'} 
                            value={manualAInput} 
                            onChange={e => handleManualAChange(e.target.value)} 
                            className={`w-full p-2 text-base font-mono font-bold border outline-none transition-colors h-[40px] ${
                              splitType === 'equal' ? 'bg-gray-100 text-gray-400 border-gray-100' : 'bg-white text-mag-black border-gray-300 focus:border-[#E91E63]'
                            }`} 
                          />
                        </div>
                        <div>
                          <label className={`text-[8px] font-black mb-1 block uppercase tracking-tighter ${splitType === 'manual' ? 'text-[#2196F3]' : 'text-gray-400'}`}>小 B 應付</label>
                          <input 
                            type="number" 
                            disabled={splitType === 'equal'} 
                            value={manualBInput} 
